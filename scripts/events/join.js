...
const fontlink = 'https://drive.google.com/u/0/uc?id=1FT-gdySMaNYPp8lrGre5sw6puUhL8qDF&export=download'
...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.postimg.cc/J0m1jKRG/Violet-Pink-Blue-Anime-Music-Twitch-Screen.gif';
let gifPath = __dirname + '/Nayan/join/join.gif';

axios.get(gifUrl, { responseType: 'arraybuffer' })
.then(response => {
    fs.writeFileSync(gifPath, response.data);
    return api.sendMessage("চলে এসেছি আমি পিচ্চি ইমরান তোমাদের মাঝে🤭!", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
🔵WHATSAPP NUMBER: wa.me/+88016899032**

🟢SUPPORT EMAIL: www.hamkarprobd@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
})
.catch(error => {
    console.error(error);
});
